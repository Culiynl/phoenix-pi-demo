    ...
    const startPolling = () => {
        const interval = setInterval(async () => {
            try {
                ...
                const winnerRes = await fetch(`${baseUrl}/api/feudal/pareto-rank`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                            ...
                            {winner ? (
                                <div style={{ display: 'flex', gap: '40px', alignItems: 'center' }}>
                                    <div style={{ width: '250px', height: '250px', background: '#fff', borderRadius: '8px', padding: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <img
                                            src={`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/api/render?smiles=${encodeURIComponent(winner.smiles)}`}
                                            alt="Winner Molecule"
