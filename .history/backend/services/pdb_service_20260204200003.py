...
class PDBManager:
    ...
    def run_p2rank(pdb_filename: str, project_id: str):
        ...
        with open(log_file, "w") as f:
            json.dump([{"time": "", "msg": "Starting P2Rank Engine...", "type": "info"}], f)

        def write_ui_log(message, msg_type="info"):
            try:
                current_logs = []
                if os.path.exists(log_file):
                    with open(log_file, "r") as f:
                        current_logs = json.load(f)
                current_logs.append({"time": "", "msg": message, "type": msg_type})
                with open(log_file, "w") as f:
                    json.dump(current_logs, f)
            except: pass

        # Command: prank.bat predict -f <file> -o <out_dir>
        # Added shell=True for Windows batch file stability
        cmd = [P2RANK_EXEC, "predict", "-f", pdb_path, "-o", output_dir]
        
        print(f"[P2Rank] Executing: {' '.join(cmd)}")
        ...
        process = subprocess.Popen(
            ...
            shell=True # CRITICAL for Windows .bat files
        )
        ...
        for line in process.stdout:
            ...
            if clean_line:
                ...
                write_ui_log(clean_line, "info")
        
        process.wait()
        if process.returncode == 0:
            write_ui_log("P2Rank analysis complete!", "success")
        else:
            write_ui_log(f"P2Rank failed with exit code {process.returncode}", "error")
    ...
    def get_p2rank_results(pdb_filename: str, project_id: str):
        # Root output dir: data/proteins/p2rank_results/{project_id}
        base_dir = os.path.join(PROTEIN_STORAGE, "p2rank_results", project_id)
        
        if not os.path.exists(base_dir):
            return None

        # Get filename stem (e.g., 6od6_protein)
        stem = pdb_filename.replace('.pdb', '').replace('.cif', '')
        
        # Based on your screenshot, P2Rank creates: base_dir / predict_{stem} / {pdb_filename}_predictions.csv
        # We use a recursive wildcard search to be 100% sure
        search_pattern = os.path.join(base_dir, "**", f"*{stem}*predictions.csv")
        print(f"[P2Rank] Searching for results with pattern: {search_pattern}")
        ...
        # Sort by mtime to get the most recent one if multiple exist
        csv_path = max(found_files, key=os.path.getmtime)
        
        try:
            print(f"[P2Rank] Success! Found results at: {csv_path}")
            df = pd.read_csv(csv_path)
            df.columns = df.columns.str.strip()
            
            # Clean numeric data for the frontend
            results = []
            ...
            print(f"[P2Rank] Error parsing results: {e}")
            return None
