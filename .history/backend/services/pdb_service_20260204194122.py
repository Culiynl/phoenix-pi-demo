...
class PDBManager:
    ...
    def get_p2rank_results(pdb_filename: str, project_id: str):
        # The directory where P2Rank was told to save (-o flag)
        base_dir = os.path.join(PROTEIN_STORAGE, "p2rank_results", project_id)
        
        if not os.path.exists(base_dir):
            return None

        # Get the "stem" (e.g., 9BA8)
        base_stem = pdb_filename.split('.')[0]
        
        # Look for ANY CSV file in that folder that contains the stem and 'predictions'
        # This covers: 9BA8.pdb_predictions.csv, 9BA8.cif_predictions.csv, 9BA8_predictions.csv
        search_pattern = os.path.join(base_dir, f"*{base_stem}*predictions.csv")
        found_files = glob.glob(search_pattern)

        if not found_files:
            print(f"[P2Rank] No CSV found matching stem: {base_stem}")
            return None
        ...
        try:
            print(f"[P2Rank] Extracting data from: {csv_path}")
            df = pd.read_csv(csv_path)
            df.columns = df.columns.str.strip()
            
            # Ensure coordinates are floats
            for col in ['center_x', 'center_y', 'center_z', 'score', 'probability']:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce')
            
            return df.dropna(subset=['center_x']).to_dict(orient="records")
        except Exception as e:
            print(f"[P2Rank] CSV Parse Error: {e}")
            return None
