...
class PDBManager:
    ...
    def get_short_path(long_path):
        """Convert a Windows path to 8.3 short path format (avoids spaces issues)"""
        buf = ctypes.create_unicode_buffer(260)
        ctypes.windll.kernel32.GetShortPathNameW(long_path, buf, 260)
        return buf.value@staticmethod
    def run_p2rank(pdb_filename: str, project_id: str):
        ...
        # Paths
        P2RANK_FOLDER = r"F:\p2rank_2.5.1"
        P2RANK_EXEC = os.path.join(P2RANK_FOLDER, "prank.bat")
        project_folder = os.path.join(P2RANK_FOLDER, "p2rank_results", project_id)
        os.makedirs(project_folder, exist_ok=True)

        # Copy PDB into project folder
        original_pdb_path = os.path.join(PROTEIN_STORAGE, pdb_filename)
        pdb_path = os.path.join(project_folder, pdb_filename)
        try:
            shutil.copy2(original_pdb_path, pdb_path)
            print(f"[P2Rank] Copied {pdb_filename} to {project_folder}")
        except Exception as e:
            print(f"[P2Rank] ERROR copying PDB: {e}")
            return

        # Log file
        log_file = os.path.join(project_folder, "p2_progress.json")
        with open(log_file, "w") as f:
            json.dump([{"time": "", "msg": f"P2Rank started for {pdb_filename}...", "type": "info"}], f)

        def write_ui_log(message: str):
            try:
        ...
        # Convert paths to short paths to avoid spaces
        pdb_path_short = PDBManager.get_short_path(pdb_path)
        output_dir_short = PDBManager.get_short_path(project_folder)
        ...
        if process.returncode == 0:
            ...
            print(f"[P2Rank] ERROR: process exited with code {process.returncode}")
