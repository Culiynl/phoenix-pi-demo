...
class PDBManager:
    ...
    def run_p2rank(pdb_filename: str, project_id: str):
        ...
        # Move the PDB file into P2Rank folder
        original_pdb_path = os.path.join(PROTEIN_STORAGE, pdb_filename)
        pdb_path = os.path.join(P2RANK_FOLDER, pdb_filename)
        try:
            shutil.copy2(original_pdb_path, pdb_path)  # copy instead of move if you want to keep original
            print(f"[P2Rank] Copied {pdb_filename} to {P2RANK_FOLDER}")
        except Exception as e:
            print(f"[P2Rank] ERROR copying PDB: {e}")
            return

        # Output directory
        output_dir = os.path.join(P2RANK_FOLDER, "p2rank_results", project_id)
        ...
        prank_exec_short = PDBService.get_short_path(P2RANK_EXEC)
