...
processed_routes = []
route_index = -1
for route_index, route_dict in enumerate(routes[:5]):
    try:
        print(f"  Processing Route {route_index+1}...")
        tree = ReactionTree.from_dict(route_dict)

        # --- SAVE IMAGE ---
        img = tree.to_image(in_stock_colors={True: "#99ff99", False: "#ffcc99"})
        fname = f'retro_{route_index}_{os.path.basename(OUTPUT_JSON)}.png'
        ...
        print(f"    -> Saved route {route_index+1}")

    except Exception as e:
        print(f"    -> Error on Route {route_index+1}: {e}")
...
"""
        ...
        try:
            ...
            if os.path.exists(json_path):
                ...
                clean_routes = []
                route_index = -1  # initialize to prevent local variable error

                for route_index, route in enumerate(routes[:5]):
                    try:
                        print(f"  Processing Route {route_index+1}...")
                        tree = ReactionTree.from_dict(route)
                        ...
                        route['image'] = fname
                        route['steps'] = steps
                        route['image_url'] = f"http://https://g5gd0v28-8000.usw3.devtunnels.ms/static/{fname}"

                        clean_routes.append(route)
                        print(f"    -> Saved route {route_index+1}")

                    except Exception as e:
                        # Only print route_index if it was actually set
                        idx = route_index + 1 if route_index >= 0 else "?"
                        print(f"    -> Error on Route {idx}: {e}")
