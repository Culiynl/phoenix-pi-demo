...
# --- CONFIG & STATE ---
# Removed the bad import 'aizynthfinder.env_call_test'
from config import (
...
class RetroManager:
    ...
    def run_retrosynthesis(smiles: str):
        ...
        # Ensure Config is Absolute Path
        ...
        # Close the file descriptor immediately, we pass the path to the worker
        json_fd, json_path = tempfile.mkstemp(suffix=".json")
        os.close(json_fd)
        
        # 3. CONSTRUCT THE WORKER SCRIPT
        # Note: We use double curly braces {{ }} to escape them for the f-string
        worker_code = f"""
import subprocess
import json
import os
...
# Handle output format (dict vs list)
routes = []
...
for i, route_dict in enumerate(routes[:5]):
    try:
        ...
        # Generate unique name based on JSON filename
        base_name = os.path.basename(OUTPUT_JSON)
        fname = f"retro_{{i}}_{{base_name}}.png"
...
# Overwrite output JSON with cleaned structure for API to read
...
"""
        ...
        try:
            ...
            # 5. Run Subprocess
            # We explicitly pass the environment python
            ...
            # Stream logs to backend console
            while True:
            ...
            if os.path.exists(json_path):
                ...
                if "routes" not in results:
                     return {"error": "Invalid JSON format from worker"}
                
                # Add Public URL for images
                for route in results["routes"]:
                    img_filename = route["image"]
                    route["image_url"] = f"http://https://g5gd0v28-8000.usw3.devtunnels.ms/static/{img_filename}"
