...
class PDBManager:
    ...
    def get_short_path(long_path: str) -> str:
        """Convert a Windows path to its short (8.3) form to avoid space issues."""
    ...
    def run_p2rank(pdb_filename: str, project_id: str):
        ...
        pdb_path = os.path.abspath(os.path.join(PROTEIN_STORAGE, pdb_filename))
        output_dir = os.path.abspath(os.path.join(PROTEIN_STORAGE, "p2rank_results", project_id))
        ...
        def write_ui_log(message):
            try:
                ...
                print(f"[P2Rank LOG ERROR] {e}")

        print("PDB EXISTS?", os.path.exists(pdb_path), pdb_path)

        # Convert paths to short paths to avoid Windows spacing issues
        pdb_path_short = PDBManager.get_short_path(pdb_path)
        output_dir_short = PDBManager.get_short_path(output_dir)
        prank_exec_short = PDBManager.get_short_path(P2RANK_EXEC)

        # Use the simpler P2Rank location
        prank_exec_short = PDBService.get_short_path(r"F:\p2rank_2.5.1\prank.bat")

        # Convert pdb and output dir to short paths too
        pdb_path_short = PDBService.get_short_path(pdb_path)
        output_dir_short = PDBService.get_short_path(output_dir)

        cmd = f'cmd /c "{prank_exec_short} predict -f {pdb_path_short} -o {output_dir_short} -v"'
        print("Running:", cmd)
        ...
        # Return path to predictions CSV (simplified: assumes single CSV in folder)
        predictions_csv = None
        csv_files = list(glob.glob(os.path.join(output_dir, "*predictions.csv")))
        if csv_files:
