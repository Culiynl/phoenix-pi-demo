...
const FeudalOptimization = () => {
    ...
    const runOptimization = async () => {
        ...
        try {
            ...
            // After optimization, auto-trigger Pareto ranking with current weights
            calculateWinner(data.history);
        } catch (e) {
                            ...
                            {winner ? (
                                <div style={{ display: 'flex', gap: '40px', alignItems: 'center' }}>
                                    <div className="stats-placeholder" style={{ width: '250px', height: '250px', background: '#fff' }}>
                                        {/* You can replace this with an <img> calling an RDKit rendering endpoint */}
                                        <span style={{color: '#000'}}>Structure: {winner.smiles.substring(0,10)}...</span>
                                    </div>
