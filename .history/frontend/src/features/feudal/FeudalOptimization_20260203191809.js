        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    setWinner(data);
                    // Update history graph if available
                    if (data.full_history) setHistory(data.full_history);
                    
                    if (data.step >= 3) { // Stop if reached max steps
                        addLog("Optimization Cycle Complete.", "success");
                        setLoading(false);
                        clearInterval(interval);
                    }
                }
            } catch (e) { /* wait for file to exist */ }
        }, 3000);
    };
