...
const Docking = () => {
    ...
    const handleImport = async (pdbId) => {
        setLoading(true);
        try {
            const res = await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/pdb/fetch/${pdbId}`);
            const data = await res.json();
            if (data.status === "success") {
                alert(`Structure ${pdbId} successfully added to Project.`);
                // Update search input to the imported ID so P2Rank is ready
                setPdbSearch(pdbId);
            }
    ...
    const handlePreview = (pdbId) => {
        // Just load the structure into Mol* without saving to project yet
        const url = `https://files.rcsb.org/download/${pdbId.toLowerCase()}.pdb`;
        // Use a generic download helper or your loadPdbIntoViewer
        loadPdbFromUrl(url);
    };
    ...
    const loadPdbFromUrl = async (url) => {
        ...
        await plugin.clear();
        // Mol* can download directly from external URLs
        const data = await plugin.builders.data.download({ url: url });
    ...
    const handleFetchPdb = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/pdb/fetch/${pdbSearch}`);
            const data = await res.json();
            if (data.status === "success") {
    ...
    const handleRunP2Rank = async () => {
        ...
        try {
            ...
            await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/p2rank/run?pdb_filename=${pdbSearch}.pdb&project_id=${projectId}`, {
                method: 'POST'
            });

            // Poll for results (Simulated delay, in production use the logging system)
            setTimeout(async () => {
                const res = await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/p2rank/results?pdb_filename=${pdbSearch}.pdb&project_id=${projectId}`);
                const data = await res.json();
                setPockets(data);
                setLoading(false);
            }, 5000);
    ...
    const loadPdbIntoViewer = async (url) => {
        ...
        const plugin = pluginRef.current;
        await plugin.clear();
        const data = await plugin.builders.data.download({ url: `http://https://g5gd0v28-8000.usw3.devtunnels.ms${url}` });
        const trajectory = await plugin.builders.structure.parseTrajectory(data, 'pdb');
        await plugin.builders.structure.hierarchy.applyPreset(trajectory, 'default');
    };
                ...
                <div className="docking-grid">
                    <div className="card" style={{ gridColumn: 'span 2' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                            <h3 style={{ fontSize: '0.8rem', color: 'var(--text-dim)', margin: 0 }}>RCSB PDB REPOSITORY SEARCH</h3>
                        ...
                        <div className="action-group">
                            <input
                                type="text"
                                placeholder="Enter keyword (e.g. Alzheimer's, Kinase, 6OD6)"
                                className="technical-input"
                                value={pdbSearch}
                                onChange={(e) => setPdbSearch(e.target.value)}
                                onKeyDown={(e) => e.key === 'Enter' && handlePdbSearch()}
                            />
                            <button className="primary-btn" style={{ background: 'var(--primary)', color: 'white' }} onClick={handlePdbSearch}>
                                Search Database
                            </button>
                        </div>

                        {searchResults.length > 0 && (
                            <div className="search-results-container" style={{ marginTop: '20px', maxHeight: '400px', overflowY: 'auto', border: '1px solid #222', borderRadius: '8px' }}>
                                <table className="technical-table">
                                    ...
                                    <tbody>
                                        {searchResults.map(result => (
                                            <tr key={result.id} style={{ cursor: 'default' }}>
                                                <td style={{ fontWeight: 'bold', color: 'var(--primary)' }}>
                                                    <a href={result.link} target="_blank" rel="noreferrer" style={{ color: 'inherit' }}>{result.id}</a>
                                                </td>
                                                <td style={{ fontSize: '0.8rem', maxWidth: '300px' }}>{result.title}</td>
                                                <td>{result.resolution ? `${result.resolution} Å` : 'N/A'}</td>
                                                <td style={{ fontSize: '0.8rem' }}>{result.organism}</td>
                                                <td>
                                                    <div style={{ display: 'flex', gap: '5px' }}>
                                                        <button className="primary-btn" style={{ padding: '4px 8px', fontSize: '0.7rem' }} onClick={() => handlePreview(result.id)}>
                                                            Preview
                                                        </button>
                                                        <button className="primary-btn" style={{ padding: '4px 8px', fontSize: '0.7rem', background: '#4ade80', color: '#000' }} onClick={() => handleImport(result.id)}>
                                                            Import
                                                        </button>
                                                    </div>
                                                </td>
