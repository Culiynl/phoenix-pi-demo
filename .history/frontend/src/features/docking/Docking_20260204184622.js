...
const Docking = () => {
    ...
    const handleImport = async (pdbId) => {
        setLoading(true);
        const cleanId = pdbId.toUpperCase();
        try {
            const res = await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/pdb/fetch/${cleanId}`);
            if (res.status === 404) throw new Error("File not found on RCSB");
            const data = await res.json();
            alert(`Structure ${cleanId} added to local storage.`);
            setPdbSearch(cleanId);
            loadPdbIntoViewer(`/static/proteins/${cleanId}.pdb`);
    ...
    const handleImport = async (pdbId) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error("PDB/CIF not found on RCSB");
            const data = await res.json();

            // data now contains { filename: "9BA8.cif", format: "cif" }
            alert(`Structure ${pdbId} successfully imported as ${data.filename}`);
            setPdbSearch(data.filename.replace(/\.[^/.]+$/, "")); // Strip extension for search state
    ...
    const handleRunP2Rank = async () => {
        ...
        setLoading(true);
        // We use the full filename now (e.g. 6OD6.pdb or 9BA8.cif)
        // If your pdbSearch state only has the ID, append .pdb as default
        const filename = pdbSearch.includes('.') ? pdbSearch : `${pdbSearch}.pdb`;

        try {
            await fetch(`http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/p2rank/run?pdb_filename=${filename}&project_id=${projectId}`, { method: 'POST' });
            ...
            const poll = setInterval(async () => {
                ...
                if (res.ok) {
                    ...
                    if (data.length > 0) {
                        const top = data[0];
                        // AUTO-SAVE CENTER FOR VINA
                        setDockingCenter({ x: top.center_x, y: top.center_y, z: top.center_z });
                        highlightPocket(top.residue_ids); // Auto-focus on #1 pocket
                    }
                    clearInterval(poll);
                    setLoading(false);
                }
