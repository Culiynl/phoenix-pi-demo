...
const Docking = () => {
    ...
    const handleRunDocking = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                console.error("--- X. Backend returned failure:", data);
                alert("Docking Failed: " + (data.detail || data.error || "Unknown error"));
            }

        } catch (e) {
            console.error("--- X. NETWORK/CODE ERROR ---", e);
    ...
    const loadStructure = async (url) => {
        ...
        try {
            ...
            console.log("--- 8. Visualization Complete ---");
        } catch (e) {
            console.error("--- X. Mol* Load Error:", e);
        }
    };
