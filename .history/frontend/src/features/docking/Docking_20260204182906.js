...
const Docking = () => {
    ...
    const handleRunP2Rank = async () => {
        ...
        try {
                ...
                try {
                    ...
                    if (res.status === 200) {
                        ...
                        if (data && data.length > 0) {
                            setPockets(data);
                            // Auto-set the center to Pocket #1
                            const top = data[0];
                            setDockingCenter({ x: top.center_x, y: top.center_y, z: top.center_z });

                            clearInterval(pollInterval);
                            setLoading(false);
                            alert("Pocket detection complete!");
                        }
                    } else {
                        // It's still 404, which means P2Rank is still working. 
                        // Do nothing and let the interval loop again.
                        console.log("P2Rank still processing...");
                    }
                } catch (err) {
            ...
            // Safety: Clear interval after 2 minutes so it doesn't poll forever if P2Rank crashes
            setTimeout(() => {
                clearInterval(pollInterval);
                setLoading(false);
            }, 120000);
            ...
            setLoading(false);
            alert("Could not start P2Rank. Check backend console.");
        }
    };
