...
function App() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch('http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/training/status');
  ...
  const fetchReview = async () => {
    ...
    try {
      const res = await fetch('http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/review', {
  ...
  const fetchChemblData = async (targetName) => {
    ...
    try {
      const res = await fetch('http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/chembl/download', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleTrainUpload = async () => {
    ...
    await fetch("http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/train", { method: "POST", body: formData });
  };
  ...
  const runRetrosynthesis = async () => {
    ...
    try {
      const res = await fetch('http://https://g5gd0v28-8000.usw3.devtunnels.ms/api/retrosynthesis', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ smiles: retroSmiles })
      });
      const data = await res.json();
      if (data.detail) alert(data.detail);
      else setRetroResults(data);
      ...
      {activeTab === 'training' && (
        <div style={styles.panel} className="card">
          ...
          <div className="metrics-grid">
            ...
            <div className="metric-box"><h4>Val R²</h4><span style={{ color: '#34d399' }}>{trainingStatus.best_r2.toFixed(3)}</span></div>
          ...
          {retroResults && (
            <div style={{ marginTop: '20px' }}>
              ...
              {retroResults.routes?.map((route, i) => (
                <div key={i} style={{ background: '#18181b', padding: '15px', borderRadius: '8px', marginBottom: '10px', border: '1px solid #333' }}>
                  <h4 style={{ margin: '0 0 10px 0', color: '#60a5fa' }}>Route {i + 1} (Score: {route.score.toFixed(2)})</h4>
                  <div style={{ fontSize: '0.9rem' }}>
                    {route.steps.map((step, s) => (
                      <div key={s} style={{ marginBottom: '5px', paddingLeft: '10px', borderLeft: '2px solid #444' }}>
                        <div style={{ color: '#9ca3af', fontSize: '0.8rem' }}>Reaction Template: {step.reaction_smarts.substring(0, 30)}...</div>
                        <div style={{ color: '#fff' }}>Reactants: {step.reactants}</div>
                      </div>
                    ))}
                  </div>
                </div>
