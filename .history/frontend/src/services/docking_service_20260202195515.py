--- START OF FILE services/docking_service.py ---
...
# Helper: Parse PDB for Center (if needed)
def calculate_pocket_center(pocket_pdb_path):
    coords = []
    if not os.path.exists(pocket_pdb_path): return DOCKING_CONFIG["CENTER"]
...
def merge_complex(protein_pdb, ligand_pdb, output_pdb):
    """Merges receptor and docked ligand."""
...
class DockingManager:
    ...
    def run_docking(project_id: str, smiles: str):
        ...
        try:
            ...
            # 3. Calculate Center (Dynamic or Fixed)
            ...
            # 6. Return Data (Paths relative to /static/ for frontend)
            # API serves /static from DATA_DIR
            # Path on disk: data/projects/123/docking/abc/complex.pdb
            # URL: /static/projects/123/docking/abc/complex.pdb
